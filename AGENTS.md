...
- Launcher was migrated **Wofi → rofi-wayland** and notifications use **SwayNC**; README.md keybinding table is **stale** (still says Wofi/Woofi-ish and old PrintScreen behavior). Trust `binds.lua` + programs over README.
...
- Screenshot binds now: `PRINT`=area→clip, `SHIFT+PRINT`=area→swappy, `SUPER+PRINT`=full→clip, `SUPER+SHIFT+PRINT`=full→swappy, `SUPER+SHIFT+P`=hyprpicker color pick.
